...
`tailor`, `cover`, `package`, `batch`, `diff`, `optimize`, `cover-score`, `fit`, `interview`, and `validate` can emit machine-readable JSON for dashboards, scripts, and CI gates. Review-oriented commands share the same top-level envelope:
